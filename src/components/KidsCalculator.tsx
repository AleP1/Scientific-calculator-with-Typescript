...
const KidsCalculator: React.FC = () => {
  ...
  return (
    <div className="max-w-md w-full mx-auto bg-pink-200 p-6 rounded-lg shadow-lg space-y-4">
      <Display value={input} className="text-3xl" />
      <div className="grid grid-cols-4 gap-4 text-black">
        {['7', '8', '9', 'DEL', '4', '5', '6', '/', '1', '2', '3', '*', '0', '.', '=', '+', 'C', '-'].map((value) => (
          <Button key={value} value={value} onClick={handleButtonClick} className="text-2xl bg-yellow-300 p-4 rounded-full hover:bg-yellow-400" />
        ))}
      </div>
    </div>
  );
